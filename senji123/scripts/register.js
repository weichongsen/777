...
document.addEventListener('DOMContentLoaded', () => {
    ...
    waitForSupabase(() => {
        console.log('Supabase 已就绪，开始检查登录状态');
        checkAuth();
    });
});
...
async function checkAuth() {
    ...
    console.log('checkAuth 被调用，client:', client);
    
    if (!client) {
        console.log('client 为 null，不执行重定向');
        return;
    }
    ...
    try {
        ...
        console.log('会话检查结果:', session ? '已登录' : '未登录');
        
        if (session) {
            console.log('已登录，跳转到首页');
            window.location.href = 'index.html';
        } else {
            console.log('未登录，保持在注册页面');
        }
    } catch (err) {
