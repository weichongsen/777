...
function bindLoginForm() {
    ...
    if (submitBtn) {
        // 移除可能存在的点击监听器
        submitBtn.removeEventListener('click', handleBtnClick);
        submitBtn.addEventListener('click', handleBtnClick);
        console.log('登录按钮点击事件监听器已绑定');
    }
}
...
async function handleLogin(e) {
    ...
    try {
        ...
        if (error) {
            ...
            console.log('登录成功！用户ID:', data.session.user.id);
            showNotification('登录成功，正在跳转...', 'success');
            
            // 确保会话已保存后再跳转
            let waitCount = 0;
            const maxWait = 20; // 最多等待 20 * 100ms = 2 秒
            
            const checkAndRedirect = async () => {
                const { data: { session: savedSession } } = await client.auth.getSession();
                
                if (savedSession || waitCount >= maxWait) {
                    if (savedSession) {
                        console.log('会话已保存，跳转到首页');
                        window.location.href = 'index.html';
                    } else {
                        console.error('会话保存失败');
                        showNotification('会话保存失败，请重试', 'error');
                    }
                } else {
                    waitCount++;
                    console.log('等待会话保存，次数:', waitCount);
                    setTimeout(checkAndRedirect, 100);
                }
            };
            
            setTimeout(checkAndRedirect, 200); // 先等待 200ms
        } else {
